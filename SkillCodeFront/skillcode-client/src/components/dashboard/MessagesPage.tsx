...
function timeLabel(dateStr: string): string {
  ...
  return `${d.getDate()} ${UA_MONTHS_SHORT[d.getMonth()]}`;
}
...
function ReceivedCard({ msg, onMarkRead, saved, onToggleSave, onShare, onTakeTest }: {
  ...
  return (
    <div className={`msg-card${msg.unread ? ' unread' : ''}`}>
      <div className="msg-card-left" onClick={() => { if (msg.unread) onMarkRead(msg.id, msg.shareId); }}>
      ...
      <div className="msg-card-right">
        <div className="msg-action-row">
          ...
          <button className="msg-btn-take" onClick={() => {
    ...
    <div className="msg-card">
      ...
      <div className="msg-card-right">
        <div className="msg-action-row">
          ...
          <button className="msg-btn-take">Пройти <span>→</span></button>
        </div>
...
export default function MessagesPage({ onTakeTest }: { onTakeTest?: (taskId: string, shareId: string) => void }) {
  ...
  list.forEach(m => { if (!seen[m.day]) { seen[m.day] = true; days.push(m.day); } });
  ...
  return (
    <div className="content">
      ...
      <div className="msg-mailbox-tabs">
        <div className={`msg-mailbox-tab${tab === 'received' ? ' active' : ''}`} onClick={() => setTab('received')}>
          <span>↓ // надіслані мені</span>
          {unreadCount > 0 && <span className="msg-tab-count">{unreadCount}</span>}
        </div>
        <div className={`msg-mailbox-tab${tab === 'sent' ? ' active' : ''}`} onClick={() => setTab('sent')}>
      ...
      <div className="msg-toolbar">
        {tab === 'received' && (
          <div className={`msg-toggle-pill${unreadOnly ? ' on' : ''}`} onClick={() => setUnreadOnly(v => !v)}>
            <span className="msg-toggle-box">{unreadOnly ? '✓' : ''}</span>
            тільки непрочитані
          </div>
        )}
